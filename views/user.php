...
<div class="mt-10 pb-20 m-2 bg-white rounded-md">
    ...
    <?php }else{?>
        ...
        <!-- Search form -->
        <form data-url="<?= $frm_search_url ?>" id="frm_search" action="/search-results" class="relative flex items-center ml-auto">
        <input name="query" type="text" class="w-full pl-7 border-4" placeholder="Search first name or last name" oninput="search_users()"
        onfocus="document.querySelector('#query_results').classList.remove('hidden')"
        onfocus="document.querySelector('#show_users').classList.add('hidden')"
